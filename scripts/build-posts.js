...
const postsDir = path.join(__dirname,'..','static-site','posts');
const outPostsDir = path.join(__dirname,'..','static-site','posts');
const outJson = path.join(__dirname,'..','static-site','assets','posts.json');
...
for (const f of mdFiles){
  ...
  // render HTML
  const mdBody = content.replace(/^---[\s\S]*?---\n/,'');
...
<body>
  <div class="site-shell">
    <header class="site-header stack">
      ...
      <div class="brand">
        <h1><a href="/">Davis Static</a></h1>
        <p>A clean and lightweight static blog</p>
      </div>
