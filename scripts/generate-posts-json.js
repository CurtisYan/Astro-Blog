...
const outFile = path.join(__dirname,'..','static-site','assets','posts.json');
...
for (const f of files){
  const rel = path.relative(path.join(__dirname,'..','static-site','posts'), f).replace(/\\/g,'/');
  ...
  let section = 'tech';
  if (parent === 'daily' || rel.indexOf('daily')>=0) section = 'life';
